...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({
    ...
    metaDescription:
      "Portal media Islam nasional modern yang menyajikan berita aktual, pemikiran moderat, dan edukasi Islam.",
  });
              ...
              {[
                {
                  label: "Nama Website",
                  key: "siteName",
                  desc: "Nama yang ditampilkan di browser dan SEO",
                },
                {
                  label: "Tagline",
                  key: "tagline",
                ...
                },
                {
                  label: "URL Website",
                  key: "siteUrl",
                  desc: "URL utama website",
                },
                {
                  label: "Email Admin",
                  key: "adminEmail",
                  desc: "Email untuk notifikasi sistem",
                },
                {
                  label: "Artikel per Halaman",
                  key: "postsPerPage",
                  desc: "Jumlah artikel per halaman listing",
                },
                {
                  label: "Google Analytics ID",
                  key: "googleAnalytics",
                  desc: "Masukkan Measurement ID (G-XXXXXXXX)",
                },
                {
                  label: "Meta Description",
                  key: "metaDescription",
                  desc: "Deskripsi default untuk SEO",
                  textarea: true,
                },
              ].map((field) => (
                <div key={field.key}>
                ...
                {[
                  {
                    ...
                    desc: "Pengunjung bisa berkomentar di artikel",
                  },
                  {
                    key: "requireModeration",
                    label: "Moderasi Komentar",
                    desc: "Komentar harus disetujui admin sebelum tampil",
                  },
                  {
                    key: "maintenanceMode",
                    label: "Mode Maintenance",
                    desc: "Nonaktifkan website untuk pengunjung umum",
                  },
                ].map((toggle) => (
                  <div
                    key={toggle.key}
                    ...
                    <button
                      onClick={() =>
                        setForm({
                          ...form,
                          [toggle.key]: !(
                            form as unknown as Record<string, boolean>
                          )[toggle.key],
                        })
                      }
                      className={`relative w-11 h-6 rounded-full transition-colors ${(form as unknown as Record<string, boolean>)[toggle.key] ? "bg-[#1a4731]" : "bg-gray-200"}`}
                    >
                      <span
                        className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${(form as unknown as Record<string, boolean>)[toggle.key] ? "translate-x-5" : "translate-x-0.5"}`}
                      ></span>
                    </button>
                  </div>
                ))}
